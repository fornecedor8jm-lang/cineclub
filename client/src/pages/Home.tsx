      ...
      <main>
        ...
        <section className={`channels-section shell ${channelsOpen ? "is-open" : ""}`} data-row="channels" aria-labelledby="channels-title">
          ...
          {channelsOpen && <>
            ...
            {!channelsLoading && !channelsError && <div className="channels-grid">{visibleChannels.slice(0, 120).map((channel) => <button type="button" className="channel-card" key={channel.id} onClick={() => setSelectedChannel(channel)} onKeyDown={(event) => { if (["Enter", " ", "Select", "OK", "DPAD_CENTER"].includes(event.key) || event.keyCode === 23) { event.preventDefault(); setSelectedChannel(channel); } }}><span className="channel-logo">{channel.logo ? <img src={channel.logo} alt="" loading="lazy" /> : <Radio size={22} />}</span><span className="channel-card-copy"><strong>{channel.name}</strong><small>{channel.group}</small></span><span className="channel-live-dot" /></button>)}</div>}
          </>}
        </section>
